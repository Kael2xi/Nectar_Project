...
const verifyEmail = async (req, res) => {
    try {
        ...
        const { email } = jwt.verify(token, process.env.jwtseceret);
...
const resendVerificationEmail = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ email: user.email }, process.env.jwtseceret, {
            expiresIn: "20mins"
        });
        const verifyLink = `${req.protocol}://${req.get(
            "host"
...
const forgotPassword = async (req, res) => {
    try {
        ...
        const resetToken = jwt.sign({ email: user.email }, process.env.jwtseceret, {
            expiresIn: "30m",
        });
